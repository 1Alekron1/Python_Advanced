...
from hw20.application import initialize_db
...
def get_students_unread_books(student):
    ...
    return [{"id": book.id, "name": book.name, "release_date": book.release_date} for book in books_by_author]


@app.route('/books', methods=['GET'])
def get_all_books():
    books = session.query(Books).options(selectinload(Books.author)).all()
    book_list = [get_book_json(book) for book in books]
    session.close()
...
def get_unread_books_by_student(student_id):
    ...
    book_list = get_students_unread_books(student)
    session.close()
    return jsonify(book_list)
...
def get_most_popular_book_among_high_achievers():
    ...
    return jsonify({"most_popular_book": get_book_json(most_popular_book)})
